...
from embodiedbench.envs.eb_manipulation.EBManEnv import EBManEnv, EVAL_SETS, ValidEvalSets, Recorder
    ...
    def evaluate(self):
        ...
        while self.env._current_episode_num < self.env.number_of_episodes:
            ...
            while not done:
                ...
                reasoning_list.append(reasoning + " Output Action: " + str(action))
                if len(action) == 0:
                    ...
                    for action_single in action[:min(self.env._max_episode_steps - self.env._current_step, len(action))]:
                        obs, reward, done, info = self.env.step(action_single, None)
