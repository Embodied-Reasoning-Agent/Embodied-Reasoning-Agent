...
if __name__ == "__main__":
    ...
    env.reset()
    print(env.episode_language_instruction)
    objects = {
        "cooled_objects" : env.env.cooled_objects,
        "heated_objects" : env.env.heated_objects,
        "cleaned_objects" : env.env.cleaned_objects,
        "visible_objs": [obj['objectType'] for obj in env.env.last_event.metadata['objects'] if obj['visible']]
    }
    print([(i, name) for i, name in enumerate(env.language_skill_set)])
    print("\nall available objects: ", objects)
    for _ in range(30):
        # Select  action
