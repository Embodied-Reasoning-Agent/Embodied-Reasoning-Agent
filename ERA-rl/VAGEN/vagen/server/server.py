...
class BatchEnvServer:
    ...
    def _setup_routes(self):
        ...
        def reset_batch():
            ...
            global_step = data['global_steps']
            results = self._reset_batch(ids2seeds, global_step)
    ...
    def _reset_batch(self, ids2seeds: Dict[str, Any], global_step: int) -> Dict[str, Tuple[Any, Any]]:
        """
        ...
        for env_name, (service, group_ids2seeds) in service_groups.items():
            print(f"Server: env_name: {env_name}")
            service_results = service.reset_batch(group_ids2seeds, global_step)
