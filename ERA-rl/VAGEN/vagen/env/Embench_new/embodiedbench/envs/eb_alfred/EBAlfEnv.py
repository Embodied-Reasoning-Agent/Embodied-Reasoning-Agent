...
class EBAlfEnv(gym.Env):
    ...
    def _reset_controller(self, task, eval_set, episode_num):
        ...
        print(f"eval_set: {eval_set}, episode_num: {episode_num}")
        print('--------------------------------')
    ...
    def reset(self, eval_set=None, episode_num=None):
        ...
        self._reset_controller(self.dataset[eval_set][episode_num], eval_set, episode_num)
    ...
    def step(self, action, action_detail, reasoning=''):
        """
        ...
        if action_detail in self.language_skill_set:
            action = self.language_skill_set.index(action_detail)
        else:
            if action < len(self.language_skill_set):
                action = action
            else:
                action = random.randint(0, len(self.language_skill_set) - 1)
        ...
        if self._current_step >= self._max_episode_steps or info['task_success']: # or self._cur_invalid_actions >= self._max_invalid_actions:
            ...
            print(f"info['task_success']: {info['task_success']}")
            print("--------------------------------")
            done = True
