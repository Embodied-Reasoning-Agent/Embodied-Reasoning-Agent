...
class TaskEnvironment(object):
    ...
    def load_config(self, task_base, waypoint_sets, config_path):
        ...
        shapes = new_base.get_objects_in_tree(exclude_base=True,
                                              object_type=ObjectType.SHAPE)
        color_info = {}
        for s in shapes:
            name = s.get_name()           # e.g. 'cube_small0_visual'
            rgb  = s.get_color()          # (r, g, b) 0~1
            color_name = self.classify_color(rgb)  # 将RGB值分类为颜色名称
            color_info[name] = color_name  # 直接使用颜色名称作为value
        print(color_info)

        return config.high_level_descriptions, self._scene.get_observation(), color_info