...
from epl.constants import IGNORE_INDEX
from epl.dataset import LazySupervisedDataset
from epl.trainer import EPLTrainer, rank0_print, safe_save_model_for_hf_trainer
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    lora: bool = field(default=False, metadata={"help": "Whether to apply LoRA."})
    lora_r: int = field(default=32, metadata={"help": "LoRA rank (r)."})
    lora_alpha: int = field(default=64, metadata={"help": "LoRA alpha."})
    lora_dropout: float = field(default=0.05, metadata={"help": "LoRA dropout."})
    lora_target_modules: Sequence[str] = field(default_factory=lambda: ["embed_tokens", "lm_head", "q_proj","v_proj","k_proj","o_proj"], metadata={"help": "LoRA target modules."})
...
class SaveLoraAdapterCallback(TrainerCallback):
    """Callback to save LoRA adapter, tokenizer and processor at each checkpoint."""
...
def train(attn_implementation="flash_attention_2"):
    ...
    if "<|think_start|>" not in additional_special_tokens:
        additional_special_tokens = additional_special_tokens + ["<|think_start|>"]
    if "<|think_end|>" not in additional_special_tokens:
        additional_special_tokens = additional_special_tokens + ["<|think_end|>"]
    if "<|action_start|>" not in additional_special_tokens:
        additional_special_tokens = additional_special_tokens + ["<|action_start|>"]
    if "<|action_end|>" not in additional_special_tokens:
        additional_special_tokens = additional_special_tokens + ["<|action_end|>"]
    ...
    if training_args.lora:
        print("Applying LoRA:", training_args.lora_r, training_args.lora_alpha, training_args.lora_dropout, training_args.lora_target_modules)
        lora_config = LoraConfig(
            r=training_args.lora_r,
            lora_alpha=training_args.lora_alpha,
            lora_dropout=training_args.lora_dropout,
            target_modules=list(training_args.lora_target_modules),
        )
    ...
    print(f"Using processor: {data_args.processor.__class__.__name__}")
    ...
    print(f"Using trainer: {trainer.__class__.__name__}")
